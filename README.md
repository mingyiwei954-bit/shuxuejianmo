...
论文共127页：摘要独占第1页，摘要、正文、AI声明与参考文献合计23页，附录从24页开始，包含指定结果表、43块完整源码及463项支撑文件清单。正式算法和五份工作簿未改变。前轮完成32段验证及32段正式期连续回放；本次是最终格式、AI说明及交付修订，没有新增数值求解。

AI声明与1页使用详情已据现有记录更新，队员须补齐实际其他工具和具体人工核验事实。当前支撑ZIP通过CRC和逐文件哈希核验，但尚未使用规范字面指定的WinRAR创建；请在人工定稿后用WinRAR重打包、计算最终MD5并按时提交。详见[提交前待办](提交前待办.md)，当前候选MD5未提交客户端。

## 完整诊断 ZIP 的无损恢复

唯一的存储形式调整：桌面 `诊断完整明细.zip` 为115,504,865字节，超过GitHub普通Git单文件100 MiB限制。两个平台都保存相同的三段字节分卷；不需要Git LFS或额外Python包。论文、源码、工作簿和小于20MB的比赛支撑包拉取后即可直接使用。
