...
AI声明与使用详情已按最新要求更新；具体人工修改、核验记录和其他工具型号仍需队员据实补充。队员真实人工核验、身份页和赛区实际提交要求仍为提交前待办；自动审计不能替代人工签核。

## 当前图表版本

本次采用5张核心图，页数和完整源码块数以publication/structural_checks.json为准。`scripts/publication_charts.py` 从冻结记录重建4张数值图，路线图为用户提供的PNG；`publication/chart_checks.json` 保存金额、单位和输入/输出哈希。绘图需要中文字体：可用环境变量 `CHART_FONT` 指向本机可用的中文TTF/OTF文件；本机使用Excel自带SimSun，其他机器须自行提供字体。本包不分发字体。无合适字体时会报错，避免静默生成中文乱码。
...
`python reproduce.py paper` 同时重建论文及 `AI工具使用详情.pdf`。正文声明读取 `论文修订源文件/ai_statement.txt`，使用详情读取 `evidence/ai_usage.json`；修改记录后请重跑生成并检查分页，勿仅手改PDF。`evidence/support_file_list.json` 是附录逐文件清单；增加/删除支撑文件时须同步此清单并重建论文。人工核验尚未完成登记，自动审计与AI排版检查不能替代。
